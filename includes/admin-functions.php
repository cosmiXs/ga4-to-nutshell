...
function ga4_to_nutshell_validate_settings($input)
{
    ...
    if (isset($input['form_user_mappings'])) {
        ...
        if (
            isset($input['form_user_mappings']['form_id']) && is_array($input['form_user_mappings']['form_id']) &&
        ...
        // If it's structured as an array of mappings
            ...
            foreach ($input['form_user_mappings'] as $mapping) {
                if (
                    isset($mapping['form_id']) && !empty($mapping['form_id']) &&
...
function ga4_to_nutshell_get_error_recovery_suggestion($error_message, $error_code, $context)
{
    ...
    if (
        strpos($error_message, 'missing') !== false ||
...
function save_ga4_nutshell_settings()
{
    if (!current_user_can('manage_options')) {
        return;
    }

    // Decode and save field mappings from JSON
    if (isset($_POST['ga4_nutshell_field_mappings'])) {
        $field_mappings = json_decode(stripslashes($_POST['ga4_nutshell_field_mappings']), true);
        if (is_array($field_mappings)) {
            foreach ($field_mappings as $form_id => $fields) {
                $field_mappings[$form_id] = array_map('sanitize_text_field', $fields);
            }
            update_option('ga4_nutshell_field_mappings', $field_mappings);
        }
    }

    // Decode and save user mappings from JSON
    if (isset($_POST['ga4_nutshell_user_mapping'])) {
        $user_mappings = json_decode(stripslashes($_POST['ga4_nutshell_user_mapping']), true);
        if (is_array($user_mappings)) {
            $user_mappings = array_map('intval', $user_mappings);
            update_option('ga4_nutshell_user_mapping', $user_mappings);
        }
    }
}
add_action('admin_init', 'save_ga4_nutshell_settings');